...
### Run n=100 simulations for N=64, D=1, P=0.1, z=0.2 and:

SARsh1 <- run(64,0.1,1,1,0.2,100)          #h=1
SARsh0.1 <- run(64,0.1,1,0.1,0.2,100)      #h=0.1
SARsh0.01 <- run(64,0.1,1,0.01,0.2,100)    #h=0.01
SARsh0.001 <- run(64,0.1,1,0.001,0.2,100)  #h=0.001


#Summarize the results

SARsh1 <- aggregate(. ~ grain , data=SARsh1,mean,na.action = na.pass)
SARsh0.1 <- aggregate(. ~ grain , data=SARsh0.1,mean,na.action = na.pass)
SARsh0.01 <- aggregate(. ~ grain , data=SARsh0.01,mean,na.action = na.pass)
SARsh0.001 <- aggregate(. ~ grain , data=SARsh0.001,mean,na.action = na.pass)
...
plot (log(SARsh1[,6]), (SARsh1[,2]), axes = FALSE, type = "o", cex=0.5,lwd = 1.5,ylim=c(0,1.1), 
      xlab= "Area of the sampling unit \n (Log Scale)", ylab= "Fraction of species remaining\n (average across sampling units)")  #SAR
axis(1, at=(log(SARsh1[,6])),SARsh1[,6])
axis(2,seq(0,1.2,0.2))
box(which = "plot")
lines(log(SARsh1[,6]),(SARsh1[,3]), col="black",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsh1[,6]),(SARsh1[,4]), col="black", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)  #Linear
lines(log(SARsh1[,6]), (SARsh0.1[,2]), type = "o", col="red",cex=0.5,lwd = 1.5)  #SAR
lines(log(SARsh1[,6]),(SARsh0.1[,3]), col="red",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsh1[,6]),(SARsh0.1[,4]), col="red", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)  #Linear
lines(log(SARsh1[,6]), (SARsh0.01[,2]), type = "o", col="blue",cex=0.5,lwd = 1.5)  #SAR
lines(log(SARsh1[,6]),(SARsh0.01[,3]), col="blue",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsh1[,6]),(SARsh0.01[,4]), col="blue", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)
lines(log(SARsh1[,6]), (SARsh0.001[,2]), type = "o", col="green",cex=0.5,lwd = 1.5)  #SAR
lines(log(SARsh1[,6]),(SARsh0.001[,3]), col="green",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsh1[,6]),(SARsh0.001[,4]), col="green", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5) #Linear

legend(6,0.3, c("SAR","cSAR","linear"),lty=c(1,2,3), lwd=c(1.5,1.5,1.5),col=c("black","black","black"),pch=c(21,22,23), bty="n")
legend(4,0.3, c("1","0.1","0.01","0.001"), lwd=c(1.5,1.5,1.5,1.5),col=c("black","red","blue","green"),pch=c(21,21,21,21), bty="n")
