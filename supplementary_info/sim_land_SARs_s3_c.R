...
### Code to draw Figure S3 (c) -  Varying h, the species affinity for the modified habitat.
...
library(sp)
  ...
  land <- function(N,P,D,h,zeta,nc,plots){   
  ...
  run<-function(N,P,D,z,n){
    ...
    for (i in 1:n)
    {
      ...
      res <- 0
      landR <- raster(plots)   
      cells<-makeClass(landR,nc,(round((N/D)*(N/D)*P)/nc))
      landR[cells]=1
      plots<-as.matrix(landR)

      
      model <- land(N,P,D,1,z,nc,plots)
      nextcol <-  data.frame(model)
      SARs10h1 <- rbind(SARs10h1, nextcol)
      
      model <- land(N,P,D,0.1,z,nc,plots)
      nextcol <-  data.frame(model)
      SARs10h0.1 <- rbind(SARs10h0.1, nextcol)
      
      model <- land(N,P,D,0.01,z,nc,plots)
      nextcol <-  data.frame(model)
      SARs10h0.01 <- rbind(SARs10h0.01, nextcol)
      
      model <- land(N,P,D,0.001,z,nc,plots)
      nextcol <-  data.frame(model)
      SARs10h0.001 <- rbind(SARs10h0.001, nextcol)
      
      print(nc)
    
      }
    
    SARs10h1 <- aggregate(. ~ grain , data=SARs10h1,mean,na.action = na.pass)  # Do not run if we want both sd and mean
    SARs10h0.1 <- aggregate(. ~ grain , data=SARs10h0.1,mean,na.action = na.pass)  # Do not run if we want both sd and mean
    SARs10h0.01 <- aggregate(. ~ grain , data=SARs10h0.01,mean,na.action = na.pass)  # Do not run if we want both sd and mean
    SARs10h0.001 <- aggregate(. ~ grain , data=SARs10h0.001,mean,na.action = na.pass)  # Do not run if we want both sd and mean
    ...
    par(mar=c(5,5,1,1)) 
    plot (log(SARs10h1[,6]), (SARs10h1[,2]), axes = FALSE, type = "o", cex=0.5,lwd = 1.5,ylim=c(0,1.1), 
          xlab= "Area of the sampling window \n (Log Scale)", ylab= "Fraction of species remaining\n (average across sampling windows)")  #SAR
    axis(1, at=(log(SARs10h1[,6])),SARs10h1[,6])
    axis(2,seq(0,1.2,0.2))
    box(which = "plot")
    lines(log(SARs10h1[,6]),(SARs10h1[,3]), col="black",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
    lines(log(SARs10h1[,6]),(SARs10h1[,4]), col="black", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)  #Linear
    lines(log(SARs10h1[,6]), (SARs10h0.1[,2]), type = "o", col="red",cex=0.5,lwd = 1.5)  #SAR
    lines(log(SARs10h1[,6]),(SARs10h0.1[,3]), col="red",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
    lines(log(SARs10h1[,6]),(SARs10h0.1[,4]), col="red", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)  #Linear
    lines(log(SARs10h1[,6]), (SARs10h0.01[,2]), type = "o", col="blue",cex=0.5,lwd = 1.5)  #SAR
    lines(log(SARs10h1[,6]),(SARs10h0.01[,3]), col="blue",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
    lines(log(SARs10h1[,6]),(SARs10h0.01[,4]), col="blue", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)
    lines(log(SARs10h1[,6]), (SARs10h0.001[,2]), type = "o", col="green",cex=0.5,lwd = 1.5)  #SAR
    lines(log(SARs10h1[,6]),(SARs10h0.001[,3]), col="green",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
    lines(log(SARs10h1[,6]),(SARs10h0.001[,4]), col="green", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)
    
    legend(6,1, c("SAR","cSAR","linear"),lty=c(1,2), lwd=c(1.5,1.5,1.5),col=c("black","black","black"),pch=c(21,22,23), bty="n")
    legend(4,1, c("1","0.1","0.01","0.001"), lwd=c(1.5,1.5,1.5,1.5),col=c("black","red","blue","green"),pch=c(21,21,21,21), bty="n")
    
}

  
### Run n=1000 simulations 
###For N=64, P=0.1, D=1 and z=0.2 and  -> Figure S3(c) 
run(64,0.1,1,0.2,1000)
