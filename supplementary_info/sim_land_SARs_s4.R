...
### Code to draw Figure S4 (a,b,c) - Varying v (degree of clustering)
...
sim<-function(N,P,D,h,z,nc,n){
  ...
  SARs <- run(N,P,D,h,z,nc,n)  #nc Cluster
  ...
  cells<-makeClass(landR,nc,(((N/D)*(N/D)*P)/nc))
  landR[cells]=1
  image(landR,axes=FALSE,xlab="",ylab="",col=c("white","grey"))
  axis(1,tick=TRUE,at=c(0,0.25,0.5,0.75,1),labels=c(1,16,32,48,64)) 
  axis(2,tick=TRUE,at=c(0,0.25,0.5,0.75,1),labels=c(1,16,32,48,64))
  axis(3,tick=TRUE,lwd.tick=0,at=c(0,1),labels=c("",""))
  axis(4,tick=TRUE,lwd.tick=0,at=c(0,1),labels=c("",""))
...
###For N=64, P=0.1, D=1, h=0.01, z=0.2 and v = 1 (one cluster) -> Figure S4(a) 
sim(64,0.1,1,0.01,0.2,1,1000)

###For N=64, P=0.1, D=1, h=0.01, z=0.2 and v = 10 (10 clusters)-> Figure S4(b)  
sim(64,0.1,1,0.01,0.2,10,1000)

###For N=64, P=0.1, D=1, h=0.01, z=0.2 and v = 410 (410 clusters)- > Figure S4(c)  
sim(64,0.1,1,0.01,0.2,410,1000)