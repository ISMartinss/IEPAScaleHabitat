...
### Code to draw Figure S3 (a) - Varying P, the amount of native habitat left in the landscape.
...
sim<-function(N,D,h,z,n){ 
  ...
  SARs10 <- run(N,0.1,D,h,z,n)  #P=0.1
  SARs50 <- run(N,0.5,D,h,z,n)  #P=0.5
  SARs90 <- run(N,0.9,D,h,z,n)  #P=0.9
  ...
  ###PLOT
  # Proportion of species remaining after 10%, 50% and 90% habitat loss - Figure S4 (a)
    par(mar=c(5,5,1,1)) 
  ...
  lines(log(SARs10[,6]),(SARs90[,4]), col="blue", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5)
  
  legend(6,1, c("SAR","cSAR","linear"),lty=c(1,2), lwd=c(1.5,1.5,1.5),col=c("black","black","black"),pch=c(21,22,23), bty="n")
  legend(4,1, c("0.1","0.5","0.9"), lwd=c(1.5,1.5,1.5),col=c("black","red","blue"),pch=c(21,21,21), bty="n")
  
  
}     

### Run n=1000 simulations 
###For N=64, D=1, h=0.01 and z=0.2 -> Figure S3(a) 
sim(64,1,0.01,0.2,1000)
