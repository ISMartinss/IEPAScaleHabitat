...
### Code to draw Figure S2 (b) - Varying z, the rate at which species richness increase with area sampled in the landscape.
...
sim <- function(N,P,D,h,zeta){
  ...
  linear_pro<-function(A1,A0,h1=h,z=zeta){
    species<-((0.4)*(A0-A1)/A0)+(A1/A0)    #Since for a modified habitat (1-sigma) = h^z ,  For sigma = 0.6  -> (1-sigma) = 0.4 #If we do not do this step (1-sigma) will change with z
    species
  }
...
### Run n=100 simulations for N=64, P=0.1, D=1, h=0.01 and:

SARsz0.1 <- run(64,0.1,1,0.01,0.1,100)   #z=0.1
SARsz0.2 <- run(64,0.1,1,0.01,0.2,100)   #z=0.2
SARsz0.3 <- run(64,0.1,1,0.01,0.3,100)   #z=0.3


#Summarize the results

SARsz0.1 <- aggregate(. ~ grain , data=SARsz0.1,mean,na.action = na.pass)
SARsz0.2 <- aggregate(. ~ grain , data=SARsz0.2,mean,na.action = na.pass)
SARsz0.3 <- aggregate(. ~ grain , data=SARsz0.3,mean,na.action = na.pass)
...
par(mar=c(5,5,1,1))     
plot (log(SARsz0.1[,6]), (SARsz0.1[,2]), axes = FALSE, type = "o", cex=0.5,lwd = 1.5,ylim=c(0,1.1), 
      xlab= "Area of the sampling unit \n (Log Scale)", ylab= "Fraction of species remaining\n (average across sampling units)")  #SAR
axis(1, at=(log(SARsz0.1[,6])),SARsz0.1[,6])
axis(2,seq(0,1.2,0.2))
box(which = "plot")
lines(log(SARsz0.1[,6]),(SARsz0.1[,3]), col="black",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsz0.1[,6]),(SARsz0.1[,4]), col="black", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5) #Linear
lines(log(SARsz0.1[,6]), (SARsz0.2[,2]), type = "o", col="red",cex=0.5,lwd = 1.5)  #SAR
lines(log(SARsz0.1[,6]),(SARsz0.2[,3]), col="red",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsz0.1[,6]),(SARsz0.2[,4]), col="red", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5) #Linear
lines(log(SARsz0.1[,6]), (SARsz0.3[,2]), type = "o", col="blue",cex=0.5,lwd = 1.5)  #SAR
lines(log(SARsz0.1[,6]),(SARsz0.3[,3]), col="blue",lty=2,pch=22, type = "o",lwd = 1.5,cex=0.5) #cSAR
lines(log(SARsz0.1[,6]),(SARsz0.3[,4]), col="blue", lty=3,pch=23,type = "o",lwd = 1.5,cex=0.5) #Linear

legend(6,0.3, c("SAR","cSAR","linear"),lty=c(1,2,3), lwd=c(1.5,1.5,1.5),col=c("black","black","black"),pch=c(21,22,23), bty="n")
legend(4,0.3, c("0.1","0.2","0.3"), lwd=c(1.5,1.5,1.5),col=c("black","red","blue"),pch=c(21,21,21), bty="n")
